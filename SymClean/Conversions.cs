    ...
    public static class Conversions
    {
        ...
        public static bool TryChangeToDateTime(object input, IFormatProvider provider, DateTimeStyles styles, out DateTime value)
        {
            ...
            if (input is long longInput)
            {
                ...
                {
                    value = new DateTime(longInput, DateTimeKind.Local);
                }
                else
                {
                    value = new DateTime(longInput, DateTimeKind.Utc);
                }
                return true;
            }
        ...
        public static bool TryChangeType(object input, Type conversionType, IFormatProvider provider, out object value)
        {
            ...
            if (IsNullable(conversionType))
            {
                ...
                if (TryChangeType(input, gType, provider, out var vtValue))
                {
        ...
        {
            if (!TryGetNumber(input, typeof(T), provider, out var obj))
            {
                value = default;
                return false;
            }
        ...
        public static bool TryGetNumber(string input, Type conversionType, IFormatProvider provider, out object value)
        {
            ...
            var ns = IsHexNumber(input, out var hexInput) ? NumberStyles.HexNumber | NumberStyles.AllowHexSpecifier : NumberStyles.Any;
            ...
            var hasDecimal = input.Contains(decSep);
            if (!hasDecimal)
            {
        ...
        {
            if (provider is not CultureInfo ci || ci.NumberFormat == null)
                return ".";
